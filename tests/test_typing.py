...
from pluginkit import AsyncPluginManager, Extension, ExtensionPoint, PluginManager, PluginResult
...
def test_caller_return_types_and_runtime() -> None:
    ...
    attributed = pm.caller(Specs.collect).collect_with_plugins(value=1)
    assert_type(attributed, list[PluginResult[str]])
    assert attributed == [PluginResult("Plugin", "v1")]
...
def test_async_caller_return_types() -> None:
    async def run() -> None:
        ...
        attributed = await pm.caller(Specs.collect).collect_with_plugins(value=2)
        assert_type(attributed, list[PluginResult[str]])
        assert attributed == [PluginResult("Plugin", "v2")]
